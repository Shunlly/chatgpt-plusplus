...
if (run.error) {
  console.error(`[!] Could not start chatgpt-plusplus: ${run.error.message}`);
  process.exit(1);
}
process.exit(run.status ?? 1);

function bootstrap() {
  console.error("[chatgpt-plusplus] First run from source install; building CLI...");
  runOrExit(npmCommand(), installArgs(), "install chatgpt-plusplus dependencies");
  runOrExit(npmCommand(), ["run", "build"], "build chatgpt-plusplus");
}
